...
def convert(source_path, out_path, target_value=COIL_PLUS_VALUE):
    ...
    # BIG-ENDIAN (">I"), confirmed 2026-09-24 across every real .sniper file in
    # this project -- unlike the rest of the file (little-endian floats
    # throughout), this trailer is stored big-endian. An earlier verification
    # pass missed this by comparing hex *strings* instead of properly
    # byte-order-decoded integers, which happened to look similar enough to
    # pass a sloppy visual check; a real-world "CRC does not match" error from
    # Holley's own software caught what that check should have. Re-verify with
    # struct.unpack(">I", ...) if this is ever touched again, not string eyeballing.
